...
Res_SNAC.rename(columns={'506_VFA_geq_PLS': 'pls_VFA_geq',
                         '506_TAN_geq_PLS': 'pls_TAN_geq',
                         '505_VFA_opt_sep': 'sep_VFA_geq',
                         '505_TAN_opt_sep': 'sep_TAN_geq'}, inplace=True)
...
All_res_normal_metainfo = pd.DataFrame(
    index=['Total analysis','Total conditions', 'pH_acid_accepted', 'pH_basic_accepted','cond_basic_accepted',
           'result provided', 'result not provided','erased analysis'], columns=['All','T1','T2','T3','T4', 'VFA', 'TAN','IC', 'TAC', 'FOS'])
...
for i in ['T1','T2','T3','T4']:
    ...
    if count!=0: # condition to avoid miscalculation when some matrices are not analysed
...
for i in ['VFA', 'TAN','IC', 'TAC', 'FOS']:
    All_res_normal_metainfo[i]['result provided'] = Res_SNAC[Res_SNAC[i+'_geq'] >= 0].shape[0]
    All_res_normal_metainfo[i]['result not provided'] = Res_SNAC[i+'_geq'].isna().sum()
...
interesting_columns=['VFA_geq','TAN_geq','IC_geq','FOS_geq','TAC_geq',
                    ...
                    'pls_TAN_geq','sep_VFA_geq','sep_TAN_geq']
...
for iii in All_res_twoindex.loc[:, 'Res_ref']:
    All_res_twoindex.insert(0, iii + '_Serie', round_value_serie(All_res_twoindex.loc[:, 'Res_ref'][iii], decimals=1))
...
for i in All_res_twoindex.index.levels[0]:# serie
...
dict_param_relation = {'VFA': ['VFA_geq', 'pls_VFA_geq', 'FOS_geq','sep_VFA_geq'],  # ref : [all param with this ref]
...
for i in All_res_ALLserie.xs('Res_ref',axis=1).columns:
    ...
    All_res_ALLserie_dict[param] = compute_profile_exactitude_global(data_global = All_res_twoindex, data_intra = All_res_INTRAserie,
                                                                     data_all = All_res_ALLserie_dict[param], param = param, eq_des = None, serie='no')
    ...
    All_res_INTERserie_dict[param] = compute_profile_exactitude_global(data_global = All_res_twoindex, data_intra = All_res_INTRAserie,
                                                                       data_all = All_res_ALLserie_dict[param], param = param, eq_des = 'des', serie='yes')
...
for i in All_res_ALLserie_dict:
    All_res_ALLserie_dict[i].to_excel(writer, sheet_name = str(i),startcol=1, startrow=1, freeze_panes=(3,2))
writer.save()
writer.close()
...
for i in All_res_INTERserie_dict:
    All_res_INTERserie_dict[i].to_excel(writer, sheet_name = str(i),startcol=1, startrow=1, freeze_panes=(3,2))
for i in All_res_INTERserie_dict_raw:
    All_res_INTERserie_dict_raw[i].to_excel(writer, sheet_name=str(i), startcol=1, startrow=1, freeze_panes=(3, 2))
# select data for livrable
# import pandas as pd
# df = pd.DataFrame()
# for i in ['VFA', 'TAN']:
#     df=pd.concat([df,All_res_INTERserie_dict[i]],axis=0)
#     for ii in ['mean','std','k_tol', 'J', 'R']:
#         if ii not in All_res_INTERserie_dict[i]:
#             pd.concat(axis=0)
writer.save()
writer.close()
